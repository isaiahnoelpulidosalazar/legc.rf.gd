...
if ($action === 'create_post') {
    ...
    // Dispatch system notice to mutual friends
    ...
    $senderName = $pdo->query("SELECT display_name FROM users WHERE id = $userId")->fetchColumn();
...
if ($action === 'add_event') {
    ...
    // Dispatch schedule update notifications to friends
    $stmtFriends = $pdo->prepare("
        SELECT friend_id AS fid FROM friendships WHERE user_id = :uid AND status = 'accepted'
        UNION
        SELECT user_id AS fid FROM friendships WHERE friend_id = :uid AND status = 'accepted'
...
// 4. FRIEND EXPLORER API endpoints
...
// 5. SETTINGS AND UTILITIES API endpoints
